...
def make_ai_move(difficulty):
    ...
    if not st.session_state.game_over and not st.session_state.board.turn:
        with st.spinner('AI is thinking...'):
            best_move = st.session_state.ai_engine.get_best_move(
                st.session_state.board, difficulty)

            if best_move:
                if st.session_state.board.is_capture(best_move):
                    captured_piece = st.session_state.board.piece_at(
                        best_move.to_square)
                    if captured_piece:
                        st.session_state.captured_pieces['white'].append(
                            captured_piece)
...
def main():
    st.set_page_config(page_title="Chess AI Agent - Stockfish-like Engine",
                       layout="wide",
                       page_icon="♟")

    st.title("♟ Chess AI Agent")
    st.markdown(
        "**Play against a strong AI using minimax search with alpha-beta pruning**"
    )
    ...
    with st.sidebar:
        ...
        if st.session_state.captured_pieces['white']:
            captured = ' '.join([
                get_piece_unicode(p)
                for p in st.session_state.captured_pieces['white']
            ])
        ...
        if st.session_state.captured_pieces['black']:
            captured = ' '.join([
                get_piece_unicode(p)
                for p in st.session_state.captured_pieces['black']
            ])
            st.markdown(f"<h2>{captured}</h2>", unsafe_allow_html=True)
        else:
            st.write("None")
    ...
    with col1:
        ...
        if not st.session_state.game_over:
            ...
            with move_input_col1:
                ...
                make_move_btn = st.button("▶️ Move",
                                          width="stretch",
                                          type="primary")
            ...
            if make_move_btn and move_input and st.session_state.board.turn == chess.WHITE:
                try:
                    ...
                    st.error(
                        "❌ Invalid move format! Use UCI notation (e.g., e2e4)")
    ...
    with col2:
        ...
        if st.session_state.game_history:
            ...
            for i in range(0, len(st.session_state.game_history), 2):
                ...
                black_move = st.session_state.game_history[i + 1].uci(
                ) if i + 1 < len(st.session_state.game_history) else ""
                moves_display.append(
                    f"{move_num}. {white_move:6s} {black_move}")

            move_history_text = "\n".join(moves_display)
            st.text_area("Move History",
                         value=move_history_text,
                         height=200,
                         disabled=True,
                         label_visibility="collapsed")
        else:
        ...
        with info_col1:
            ...
            st.metric("Legal Moves",
                      st.session_state.board.legal_moves.count())
